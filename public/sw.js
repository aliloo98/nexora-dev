      ...
      return Promise.all(
        ASSETS.map(asset => {
          return cache.add(asset).catch(err => {
            console.warn('Asset non précaché', asset, err);
          });
        })
      );
    })
...
self.addEventListener('fetch', e => {
  ...
  if (url.pathname.endsWith('manifest.json')) {
    const iconTheme = url.searchParams.get('icon');
    const iconPrefix = iconTheme ? `/icon-${iconTheme}` : '/icon';
    e.respondWith(
      fetch(e.request)
