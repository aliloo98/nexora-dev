...
const planMarkupBuilder = readSource('../plan/planMarkupBuilder.js')
const main = readSource('../main.js')
const coupleSectionRenderer = readSource('../couple/coupleSectionRenderer.js')
...
assert.match(planMarkupBuilder, /escapeHtml\(item\.title \|\| \(positive \? 'Revenu' : 'Charge'\)\)/, 'plan rows must escape movement titles')
assert.match(coupleSectionRenderer, /escapeHtml\(household\.name \|\| 'Foyer Nexora'\)/, 'couple summary must escape household names')
assert.match(coupleSectionRenderer, /escapeHtml\(goal\.name \|\| 'Objectif'\)/, 'couple summary must escape goal names')
assert.match(coupleSectionRenderer, /escapeHtml\(debt\.name \|\| 'Dette'\)/, 'couple summary must escape debt names')
assert.match(coupleSectionRenderer, /data-share-id="\$\{escapeHtml\(id\)\}"/, 'sharing controls must escape imported identifiers')
