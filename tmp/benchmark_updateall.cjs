...
(async () => {
  ...
  const result = await page.evaluate(({ headSource }) => {
    const ids = [
    ...
    const ensureHeadUpdateAll = () => {
      if (typeof window.__headUpdateAll === 'function') return;
      window.__headUpdateAll = undefined;
      eval(headSource);
      if (typeof window.__headUpdateAll !== 'function') {
        throw new Error('HEAD updateAll injection failed');
      }
    };
    ensureHeadUpdateAll();
    ...
    const instrumentAndMeasure = (fn) => {
      ...
      return { counters, times, stats: { mean, min: Math.min(...times), max: Math.max(...times) } };
    };
    ...
    return { currentMetrics, headMetrics, currentAfter, headAfter, diff, currentBefore, pageTitle: document.title, url: location.href };
  }, { headSource });
